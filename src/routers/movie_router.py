...
# CREATE
@movie_router.post("/", status_code=201, response_model= List[Movie], tags=['Movies'])
...
# UPDATE
@movie_router.put("/{id}", response_model=Movie, tags=['Movies'])
...
# DELETE
@movie_router.delete("/{id}", response_model= List[Movie],  tags=['Movies'])
...
# ALL MOVIES
@movie_router.get("/", response_model= List[Movie], tags=['Movies'])
async def get_movies() -> List[Movie]:

    return movies

# MOVIE BY CATEGORY
@movie_router.get("/category", response_model= List[Movie], tags=['Movies'])
...
# MOVIE BY ID
@movie_router.get("/{id}", response_model=Movie, tags=['Movies'])
